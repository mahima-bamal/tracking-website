...
def valid_email(email):
    ...
    try:
        ...
        s.sendmail(sender_email_id,receiver_email_id, "hello world")
        s.quit()
        return True
    except:
        return False

# File to store competitor data
...
if 'competitors' not in st.session_state:
    st.session_state.competitors = load_data() or [{} for _ in range(4)]

def trend_analysis():
    if st.button("Back to Home", key="trend_analysis_back"): # Back button
    ...
    def verified_row(row_index):
        ...
        save_data(st.session_state.competitors)
    ...
    for row in range(len(st.session_state.competitors)):
        with st.container():
            ...
            with cols[0]:
                username = st.text_input('Username', key=f'username_{row}', value=st.session_state.competitors[row].get('username', ''))
                st.session_state.competitors[row]['username'] = username
            with cols[1]:
            ...
            with cols[3]:
                if st.button('Verify', key=f'verify_{row}', use_container_width=True):
                    with st.container():
                        if not youtube and not instagram: #check if both fields are empty
                            st.session_state.competitors[row]['youtube'] = ""
                            st.session_state.competitors[row]['instagram'] = ""
                            save_data(st.session_state.competitors)
                            continue
                        
                        if youtube: #validate youtube handle
                            ...
                            st.session_state.competitors[row]['youtube'] = ''
                        if instagram: #validate instagram handle
                            ...
                            st.session_state.competitors[row]['instagram'] = ''
                        #proceed with verification if at least one handle is valid
    ...
    with col2:
        if st.button("Summarize"):
            ...
            for username in verified_instagram_username:
                if username:
                    try:
                        posts = get_post_uploaded_instagram(username)
                        summerized_insta_captionn_likes.extend(posts)
                    except Exception as e:
                        st.error(f"Error fetching Instagram data for {username}: {str(e)}")
            ...
            try:
                ...
                message = (
                    ...
                    instagram_trend="90s nostalgia",
                    instagram_recommend="To join this trend, create content that evokes a sense of nostalgia for the 1990s. This could include fashion, music, movies, toys, and other pop culture elements from that decade. Use relevant hashtags such as #90sNostalgia, #90sFashion, #90sMusic, etc. You can also incorporate popular 90s slang or references in your captions to further engage your audience. Consider a '90s themed photoshoot or video, recreating iconic scenes or looks from the era. Collaborate with other creators who share a similar interest in 90s culture to expand your reach.",
                    youtube_trend="90s nostalgia",
                    youtube_recommend="Create content related to 90s fashion, music, movies, and TV shows. Use popular 90s soundtracks in your videos. Collaborate with other creators on 90s-themed content. Engage with viewers by asking them to share their favorite 90s memories."
                )
                ...
                s.send_message(msg)
                s.quit()
                st.success(f"Summary sent to {receiver_email_id} successfully!")
            except Exception as e:
                st.error(f"Failed to send email: {str(e)}")
                
            # Display results
            st.write("**Instagram Trends**:", instagram_trend)
            st.write("**Instagram Recommendations**:", instagram_recommend)
            st.write("**YouTube Trends**:", youtube_trend)
            st.write("**YouTube Recommendations**:", youtube_recommend)
        else:
            st.write("No summary .")
